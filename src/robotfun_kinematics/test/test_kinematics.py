#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tests del núcleo de cinemática (5 juntas + gripper, modos bloqueado/roll).
...
Cubren las condiciones de validación del proyecto:
  ...
  * IK analítica exacta (modo bloqueado) e IK numérica (ambos modos).
  * El modo roll activo alcanza direcciones fuera del plano que el modo
    bloqueado no puede.
"""
...
from robotfun_kinematics.core import (
    A3, L0, L1, L2, L5, N_JOINTS, ROBOT, ROLL_INDEX, WorkspaceLimits,
    approach_angle, approach_direction, clamp_target, joint_trajectory,
    solve_analytic, solve_ik, solve_numeric, trapezoidal_profile,
    validate_target,
)
...
# Cinemática directa
# ---------------------------------------------------------------------------
def test_n_joints_is_five():
    assert N_JOINTS == 5


def test_fk_home_matches_expected():
    """FK(HOME=q0): brazo recto vertical (tabla DH del usuario)."""
    ...
    np.testing.assert_allclose(fr[3][:3, 3], [0.010, 0.0, 0.303], atol=1e-9)  # muñeca
    np.testing.assert_allclose(fr[4][:3, 3], [0.010, 0.0, 0.393], atol=1e-9)  # TCP


def test_joint4_is_pure_roll_at_home():
    """Girar joint_4 en HOME NO debe trasladar el TCP (solo rota la muñeca)."""
    p0 = ROBOT.tool_position(np.zeros(N_JOINTS))
    for q4 in np.linspace(-np.pi / 2, np.pi / 2, 13):
...
def test_approach_angle_identity():
    """φ(q) = π/2 + q2 + q3 − q5 exacto con q4=0 (identidad de la tabla DH)."""
...
def _fk_urdf(q):
    ...
    from robotfun_kinematics.core import L3, L4
    return (_R((0, 0, 1), q[0])                              # joint_1 yaw
            @ _T([L0, 0, L1]) @ _R((0, -1, 0), q[1])         # joint_2 hombro
            @ _T([0, 0, L2]) @ _R((0, -1, 0), q[2])          # joint_3 codo
            @ _T([0, 0, L3]) @ _R((0, 0, 1), q[3])           # joint_4 roll
            @ _T([0, 0, L4]) @ _R((0, 1, 0), q[4])           # joint_5 pitch pinza
            @ _T([0, 0, L5]))                                # tool0 (TCP)
...
# ---------------------------------------------------------------------------
# IK — modo joint_4 BLOQUEADO (analítica y numérica)
# ---------------------------------------------------------------------------
...
def test_analytic_ik_exact():
    """La IK analítica reproduce posición y φ exactos, con q4=0."""
    for (x, y, z) in _reachable_front_targets(40):
        res = solve_analytic(x, y, z, PHI_DOWN)
        assert res.ok and res.mode == "locked"
        assert abs(res.q[ROLL_INDEX]) < 1e-12
...
def test_numeric_ik_locked_with_warm_seed(method):
    """DLS y Newton convergen a posición+φ con semilla pick-ready y q4 fijo."""
    ...
    for (x, y, z) in targets:
        res = solve_ik([x, y, z], seed, approach=PHI_DOWN, method=method,
                       lock_joint_4=True)
        assert abs(res.q[ROLL_INDEX]) < 1e-12          # el roll no se mueve
...
# ---------------------------------------------------------------------------
# IK — modo joint_4 ACTIVO (posición + dirección de apuntado 3D)
# ---------------------------------------------------------------------------
def _angle_between(a, b):
    return np.arccos(np.clip(np.dot(a, b), -1.0, 1.0))


def test_roll_mode_reaches_out_of_plane_direction():
    """Con roll activo se apunta FUERA del plano del brazo; bloqueado, no.

    El objetivo se genera por FK desde una postura con roll grande (factible
    por construcción y dentro de límites)."""
    q_true = np.array([0.30, 0.45, 0.50, 0.80, -0.70])
    ...
    assert res.ok and res.mode == "roll"
    assert np.linalg.norm(ROBOT.tool_position(res.q) - target) < 1e-3
    assert _angle_between(ROBOT.tool_axis(res.q), d_des) < np.radians(0.5)
    assert abs(res.q[ROLL_INDEX]) > np.radians(3)      # el roll trabaja de verdad

    res_l = solve_numeric(target, seed, dir_des=d_des, lock_joint_4=True)
    assert abs(res_l.q[ROLL_INDEX]) < 1e-12
    assert _angle_between(ROBOT.tool_axis(res_l.q), d_des) > np.radians(3)


def test_roll_mode_with_phi_matches_locked_positions():
    """Con φ en el plano, el modo roll llega a los mismos puntos que el bloqueado."""
    seed = solve_analytic(0.18, 0.0, 0.10, PHI_DOWN).q
    for (x, y, z) in _reachable_front_targets(15, seed_rng=5):
        res = solve_ik([x, y, z], seed, approach=PHI_DOWN, method="dls",
                       lock_joint_4=False)
        assert np.linalg.norm(ROBOT.tool_position(res.q) - [x, y, z]) < 1e-3
        d_des = approach_direction(x, y, PHI_DOWN)
        assert _angle_between(ROBOT.tool_axis(res.q), d_des) < np.radians(1.0)


def test_direction_from_approach_helper():
    d = approach_direction(0.2, 0.0, -np.pi / 2)
    np.testing.assert_allclose(d, [0, 0, -1], atol=1e-12)
    d = approach_direction(0.2, 0.0, 0.0)
...
def test_joint_trajectory_endpoints():
    ...
    qg = np.array([0.4, -0.3, 0.2, 0.0, 0.5, 0.3])
